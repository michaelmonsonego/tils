...
FetchData(T_cells, vars = c("CD8A", "CD8B")) #M# just to see cd8 expression
...
T_cells_0 <- subset(T_cells_0, subset = CD8A < 0.5 & CD8B < 0.5)
table(T_cells_0$orig.ident)
table(T_cells_0$seurat_clusters)
all_features <- rownames(T_cells_0)
print("CD8A" %in% all_features)
print("CD8B" %in% all_features)
...
T_cells_0 = NormalizeData(T_cells_0,assay = "RNA",normalization.method = "LogNormalize", scale.factor = 10000) #M# not working. wonder why
...
T_cells_0 <- FindClusters(T_cells_0, resolution = 0.4)
#saveRDS(T_cells_0, file = "objects/tils_0_.4.rds")
T_cells_0 <- readRDS("objects/tils_0_.4.rds")
```
